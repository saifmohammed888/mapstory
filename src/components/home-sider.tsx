...
const HomeSider: React.FC<HomeSiderProps> = () => {
    ...
    const filteredTrips = useMemo(() => {
        ...
        if (searchText.length > 0) {
            filteredData = trips.filter(
                (trip) => trip.title.toLowerCase().search(searchText.toLowerCase()) !== -1
        ...
        if (tripSort === TripSort.BY_DATE_ASC) {
            filteredData.sort(function (a, b) {
                return a.visitStartDate > b.visitStartDate ? 1 : a.visitStartDate < b.visitStartDate ? -1 : 0
            })
        }
    ...
    return (
        <Sider className="app-sider home-sider" width={340}>
            <Input placeholder="input search text" onChange={onSearch} suffix={<SearchOutlined />} />
            <Divider />
            <Row gutter={16}>
                <Col span={12}>
                    <Popover
                    ...
                    >
                        <Button className="filter-btn">
                            {typeFilter.length > 0 ? `${typeFilter.length} Filer Applied` : "Select Filter"} <FilterOutlined />
                        </Button>
                    </Popover>
                </Col>
                <Col span={12}>
            ...
            <div className="trip-list-meta">
                <div>
                    {filteredTrips.length === 0 && (searchText !== "" || typeFilter.length > 0) ? (
                        <Badge count="No trip meet the search / filter criteria." />
                    ) : (
                        ""
                    )}
                </div>
                <Badge count={`${filteredTrips.length} Trip${filteredTrips.length > 1 ? "s" : ""}`} style={{ background: "#1890ff" }} />
            </div>
